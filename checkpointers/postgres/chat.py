"""
LangGraph + PostgreSQL Checkpointer - CLI Demo
==============================================
Shows how checkpoints are stored in a real Postgres database.

Prerequisites:
  docker compose up -d          (start local Postgres)
  OR set DATABASE_URL to any Postgres connection string (e.g. Supabase)
...
def cmd_chat(args):
    ...
    db_host = DATABASE_URL.split("@")[-1].split("/")[0] if "@" in DATABASE_URL else DATABASE_URL
    print(f"\nPostgres chatbot  |  thread='{args.thread}'  |  db={db_host}")
...
def cmd_state(args):
    ...
    try:
        ...
        if not snap or not snap.values:
            ...
            return
        _print_snapshot(snap, label=f"Thread '{args.thread}'  [PostgreSQL]")
    finally:
        conn.close()
...
def cmd_history(args):
    ...
    try:
        ...
        if not history:
            ...
            return
        print(f"\n{'='*60}")
        print(f"  Checkpoint history — thread '{args.thread}'  [PostgreSQL]")
...
def cmd_threads(args):
    ...
    print(f"\n{'='*60}")
    print(f"  All threads  [PostgreSQL]")
...
def main():
    parser = argparse.ArgumentParser(
        description="LangGraph + PostgreSQL checkpointer demo",
    ...
    sub.add_parser("threads", help="List all threads in the database")
